...
### 2026-06-14 — Brief: product/PM lens, NVIDIA-PM prep, repo crawl, latest-literature pass
Expanded the master brief per request: (1) folded in the **product/ecosystem map** (user journeys + pain→opportunity table + the inference-vendor-PM persona + the C5 product-vs-paper note) in §4; (2) added a new **§8 "For the NVIDIA inference-PM role"** — Dynamo in depth (KV-aware router, agent hints, ATIF roadmap), what an inference PM owns, how GRIT findings feed product decisions, and a "questions to answer cold" study box (renumbered Open-questions→§9, Next-actions→§10); (3) a **GitHub repo crawl** table in §6 (real star counts, 2026-06-14: AutoGPT 185k, llama.cpp 117k, vLLM 83k, OpenHands 77k, SGLang 29k, Dynamo 7.3k, Mooncake 5.6k, …); (4) a **latest-literature pass** (§5g): GORGO `2602.11688`, Pie (SOSP'25), KVFlow=NeurIPS'25 venue, ProphetKV/Prefill-as-a-Service, plus a watch-list — all added to the ledger (pass-4 block). Bumped "Last updated" to 17:22 PDT; regenerated PDF; verified 10 balanced sections.

### 2026-06-14 — Brief: full body re-sync into the master educational document
Rewrote `docs/agentic-inference-brief.html` end-to-end as the **master brief** (9 sections, TOC): (1) exec summary; (2) how LLM inference works — the mental model; (3) what an agent is + patterns; (4) the ecosystem + the orchestrator↔engine seam + personas; (5) the full annotated literature (foundations → frontier → measurement/cost → benchmarks/traces → industry), every citation ✓-checked against the ledger; (6) the tools; (7) the GRIT project — problem, contributions, **the same-stream-counterfactual metric**, hypotheses, pilot, the two BLOCKERs, statistical-soundness caveat, threats, honest status incl. the Codex reject + how it's answered; (8) open questions / benchmark↔production gap / who to talk to; (9) next actions + reading queue + quarantined unverified citations. Reflects all recent changes (GRIT naming, GAIATrace added, GoodServe correction, softened venues, no "suspended"). Regenerated the PDF; bumped the "Last updated" stamp to 17:13 PDT. Tag-balance + render verified.

### 2026-06-14 — Brief masthead refresh + "last updated" timestamp (standing rule)
Retitled the study brief to **GRIT** and added a prominent **"Last updated: 2026-06-14 17:09 PDT"** line at the top (`.deck`) and footer; regenerated `docs/agentic-inference-brief.pdf` from the HTML so the two stay in sync. Standing rule going forward: the brief is refreshed + re-timestamped on every substantive change (recorded in session memory; not hook-automatable since it needs content regeneration).

### 2026-06-14 — External Codex review (pass 3) triage + Figure-1 B3 alignment
Triaged a hostile Codex/area-chair review (run by the user in terminal). **Verified-and-fixed (integrity):** corrected a **GoodServe** mischaracterization (it *predicts output length*, task type is a feature — not "infers task type"); softened unconfirmed venue labels (**Cost-of-Pass** ICLR'26 → OpenReview submission; **CONCUR** ICML'26 → "claimed/unconfirmed"); removed an unsourced **FlashMemory "project suspended"** claim (3 files); flagged the **vLLM×Mooncake figures** as lacking a primary URL; **added GAIATrace `2606.01725`** (✓ web-verified — a real public agent-trace prior Codex caught missing); fixed the stale **brief** footnote that still called Agent Memory/ThunderAgent/GoodServe/CONCUR unverified and still cited 600-run/<$300. **Accepted-and-encoded (methodology):** the **locality tax must be a same-stream oracle-vs-bounded counterfactual** (Δ GPU-seconds/verified task), not a "share of the gap" (`metric-design.md`, pilot #20); **two scope-matched estimands** because realized can exceed own-lineage available (#22); **binary-success denominator**, partial credit supplementary, no task pre-selection; **per-tenant eviction attribution is an engine *patch* not "a few lines"** and gates C1 (`observability-and-power.md` §1); **retracted the "1,200 trajectories = 80% power" claim** (no variance/MDE yet); **`retain_during_tool` is a mechanism not a stock knob** (#21); **H1 needs a same-stream controlled-eviction design**, not isolated-vs-mixed. Added **threats #10–18** (saturation regime, chat-SLO harm, synthetic-mixture, instrumentation perturbation, quantization/block-size/routing, censoring, scaffold selection, multiple comparisons, oracle error). Fixed the `run_pilot.main()` benchmark-list/axis bug. **Verdict noted:** Codex says *reject* until the causal counterfactual exists — which is exactly the C1 primitive now written into `metric-design.md`. Also fixed Figure-1: the two-line "SWE-bench…" box broke measurement-band alignment → retitled "Task benchmarks" + uniform meas-box height (verified via headless render).
